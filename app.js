...
const fetchTwelveDataQuotes = symbols =>
  new Promise(resolve => {
    const apiKey = process.env.TWELVE_DATA_API_KEY;
    const filtered = symbols.filter(Boolean);
    if (!apiKey || !filtered.length) {
      resolve([]);
      return;
    }
        ...
        response.on('end', () => {
          try {
            ...
            const results = Array.isArray(payload)
              ? payload
              : Object.values(payload).filter(item => item && item.symbol);
            resolve(
              results.map(item => ({
                symbol: item.symbol,
                price: item.close ? Number(item.close) : Number(item.price),
                change: item.change ? Number(item.change) : null,
                changePercent: item.percent_change ? Number(item.percent_change) : null,
...
const fetchQuotes = async symbols => {
  const twelveDataQuotes = await fetchTwelveDataQuotes(symbols);
  if (twelveDataQuotes.length) {
